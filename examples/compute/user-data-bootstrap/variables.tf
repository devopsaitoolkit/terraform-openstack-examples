...
variable "instance_name" {
  description = "Name of the compute instance. Also passed into the bootstrap template as the hostname."
  type        = string
  default     = "example-bootstrap"
}
...
variable "motd" {
  description = "Message-of-the-day string rendered into the bootstrap script and written to /etc/motd."
  type        = string
  default     = "Provisioned by Terraform + OpenStack user-data."
}

variable "tags" {
  description = "Tags applied to the instance for inventory and cost tracking."
  type        = list(string)
  default     = ["managed-by:terraform", "example:user-data-bootstrap"]
}
